...
import exception.AccountWithInvestment;
import exception.InvestmentNotFound;
...
public class InvestmentRepository {
    ...
    public InvestmentWallet initInvestment(final AccountWallet account, final long id){
        if (!wallets.isEmpty()) {
            ...
            if (accountsInUse.contains(account)) {
                throw new AccountWithInvestment("A conta'" + account + "'já possui um investimento");
            }
        }
    ...
    public Investment findById(final long id){
        return investments.stream().filter(a -> a.id() == id)
                .findFirst()
                .orElseThrow(
                        () -> new InvestmentNotFound("O investimento '" + id + "' não foi encontrado")
                );
    }
    ...
    public InvestmentWallet findWalletByAccountPix(final String pix){
        return wallets.stream()
                ...
                .findFirst()
                .orElseThrow(
                        () -> new WalletNotFound("A carteira não foi encontrada")
                );
    }
