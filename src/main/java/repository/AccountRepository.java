...
public class AccountRepository {
        ...
        if (!accounts.isEmpty()) {
            ...
            for (var p : pix) {
                if (pixInUse.contains(p)) {
                    throw new PixInUse("O pix '" + p + "'já está em uso");
                }
            }
        }
    ...
    public AccountWallet findByPix(final String pix){
        return accounts.stream()
                ...
                .findFirst()
                .orElseThrow(() -> new AccountNotFound("A conta com a chave pix '" + pix + "' não existe ou foi encerrada"));
    }
